...
let view_of_square = (~inject, ~index: int, square: Model.square): Vdom.Node.t =>
  ...
  | None =>
    ...
    Vdom.Node.div(
      ~attr=
        Vdom.Attr.many([Vdom.Attr.classes(["square"]), ...click_handlers]), //only empty square can be clicked
      [PlayerMark.view(square)],
    );
  | Some(_) =>
    Vdom.Node.div(
      ~attr=Vdom.Attr.many([Vdom.Attr.classes(["square"])]),
      [PlayerMark.view(square)],
    )
  };
...
let view = (~inject, model: Model.t) => {
  //TODO
  Vdom.Node.div(~attr=Vdom.Attr.id("board"), []);
};
